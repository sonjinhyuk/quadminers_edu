...
class Flow:
    ...
    def get_payload(self, packet) -> bytes:
        if packet.getlayer("Raw"):
            try:
                # load_value = packet.getlayer(TCP).payload.getlayer("Raw").show(dump=True)
                load_value = packet.getlayer(TCP).payload.getlayer("Raw").load
            except AttributeError:
                # load_value = packet.getlayer("Raw").show(dump=True)
                load_value = packet.getlayer("Raw").load

            self.payload += load_value
            # load_value = load_value.split("###[ Raw ]###")[-1].split("load      = ")[-1].replace("\\t", "").replace(
            #     "\\n", "").replace("\\r", "")
            # load_value = load_value.replace("\t", "").replace("\n", "").replace("\r", "")

            # if self.translation_port != -1:
            #     dport = self.translation_port
            # else:
            #     dport = packet.dport
            # decoded, port = decode_payload(dport, load_value)
            # # for b in load_value:
            # #     if 32 <= b <= 126:
            # #         decoded += chr(b)
            # #     elif b == 0:
            ...
            # ports = [80, 8080, 443, 53, 20, 21, 25, 143, 110, 67, 68, 123, 5355, 139, 445, 161, 162, 137, 138, 5353,
            #          389, 636, 135, 88, "TLS", "TLS/PKI", "LDAP"]
    ...
    def add_packet(self, packet: Any, direction: Enum) -> None:
        ...
        self.get_payload(packet)
        # cleaned_payload = re.sub(r"\\x[0-9a-fA-F]{2}", "", self.payload)
        # cleaned_payload = re.sub(r"\\x(?!00)[0-9a-fA-F]{2}", "", self.payload).replace("\\x00", " ").replace("\\", "")
