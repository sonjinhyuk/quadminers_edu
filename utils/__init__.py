...
import pandas as pd
import numpy as np
from utils.TH_BERT import BERTClass
...
def data_load(base_dir="./model_data/",
    ...
    if scale:
        ...
        if scaler_path is None:
            scaler = MinMaxScaler()
            scaler_path = f"output_models/minmax_scaler.pkl"
...
def validation_model(model, test_data, device, model_type="bert", pred_value_check=False):
    if model_type == "bert":
        ...
        acc = accuracy_score(xgboost_y, pred)
        presision = precision_score(xgboost_y, pred, average='weighted')
        recall = recall_score(xgboost_y, pred, average='weighted')
        f1 = f1_score(xgboost_y, pred, average='weighted')
        cm = confusion_matrix(xgboost_y, pred)
        return [acc, presision, recall, f1], cm, xgboost_y
